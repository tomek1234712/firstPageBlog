...
final class ImageJobs extends FileJobs
{
  ...
  */
  public function createCustomThumb( $sImgSource, $sImgDestDir, $iSize = null, $sImgOutput = false, $bOverwrite = null, $iCrop = null ){
    ...
    if( $this->checkImgMaxDimension( $sImgSource ) ){
      ...
      $sFile = $this->createThumb( $sImgSource, $sImgDestDir, $sImgOutput, IMAGE_QUALITY, $iCrop );
  ...
  */
  public function createThumb( $sImgSource, $sImgDestDir, $sImgOutput = false, $iQuality = null, $iCrop = null ) {
    ...
    if ( !$aImgProperties[2] == 2 ) {
      ...
      if( isset( $GLOBALS['config']['crop_options'] ) && isset( $iCrop ) && is_numeric( $iCrop ) && isset( $GLOBALS['config']['crop_options'][$iCrop][2] ) ){
        list( , $fCropFile, $fCropRatio ) = $GLOBALS['config']['crop_options'][$iCrop];
        $iRatio = $iImgCreateY / $iImgCreateX;
      }

      if( isset( $iRatio ) && $iRatio != $fCropRatio ){
        if( $fCropRatio > 1 ){
          $this->iThumbY = $this->iThumbX;
          $this->iThumbX = ( $iRatio > $fCropRatio ) ? $this->iThumbY / $fCropRatio : $this->iThumbY / $fCropRatio;
        }
        else{
          $this->iThumbY = ( $iRatio > $fCropRatio ) ? $this->iThumbX * $fCropRatio : $this->iThumbX * $fCropRatio;
        }
        if( $iRatio > $fCropRatio ){
          $iCropYStart = (int) ( ( $iImgCreateY / 2 ) - ( ( $iImgCreateX * $fCropRatio ) / 2 ) );
          $iImgCreateY = (int) ( $iImgCreateX * $fCropRatio );
        }
        else{
          $iCropXStart = (int) ( ( $iImgCreateX / 2 ) - ( ( $iImgCreateY / $fCropRatio ) / 2 ) );
          $iImgCreateX = (int) ( $iImgCreateY / $fCropRatio );
        }
      }
      else{
      ...
      if( !$sCreateFunction( $mImgDest, $mImgCreate, 0, 0, ( isset( $iCropXStart ) ? $iCropXStart : 0 ), ( isset( $iCropYStart ) ? $iCropYStart : 0 ), $this->iThumbX, $this->iThumbY, $iImgCreateX, $iImgCreateY ) ) {
