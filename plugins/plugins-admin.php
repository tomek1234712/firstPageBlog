...
/**
* Function returns code for CKEditor wysiwyg editor
* @return string
* @param  string  $sName
* @param  string  $sContent
* @param array $aParametersExt
* Default options: sToolbar, sPlugins
*/
function getWysiwygCKEditor( $sName, $aParametersExt ){
  $content = null;
  if( !defined( 'CKEDITOR_WYSIWYG_START' ) ){
    define( 'CKEDITOR_WYSIWYG_START', true );
    $content .= '<script src="plugins/ckeditor/ckeditor.js"></script>';
  }

  $content .= '<script>$( function(){ CKEDITOR.replace( "'.$sName.'", { language: "'.$GLOBALS['config']['admin_lang'].'" } ); } )</script>';
  return $content;
} // end function getWysiwygCKEditor
